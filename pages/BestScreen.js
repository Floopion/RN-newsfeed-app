...
export default function HomeScreen() {
  ...
  const [content, setContent] = useState(<Text style={styles.load}>Loading Best Stories %0</Text>);
   ...
   //set the loading gif
    ...
    .then(async function (response) {
      ...
      for(let i in all ){
        ...
        setContent(
            <Text style={styles.load}>Loading Best Stories %{i}</Text>
          )
      }
    })
    ...
    .then(function () {
      //once everything has been done, cook the loading gif, and run the function to set the content.
      setloading(null) 
      setCards();
    });
  }
